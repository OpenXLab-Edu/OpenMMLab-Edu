...
def only_infer_demo():
	...
	# 实例化模型，网络名称为'Imporved_DDPM'
	model = gen(backbone="Imporved_DDPM", tpye ="", dataset_path="dataset/edges2shoes")
	# 指定权重文件的路径
...
def normal_train_demo():
	# 实例化模型，网络名称为'Imporved_DDPM'
	model = gen(backbone='Imporved_DDPM')
	# 指定数据集的路径
...
def continue_train_demo():
	...
	model.save_fold = "checkpoints/gen_model"
	# 指定预训练模型的权重文件
	checkpoint='../checkpoints/gen/latest.pth'
